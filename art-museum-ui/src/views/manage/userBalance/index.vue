...
export default {
  ...
  data() {
    return {
      ...
      form: {},
      // 导出地址
      exportUrl: 'manage/userBalance/export',
      // 表单校验
      rules: {
        userId: [
  ...
  methods: {
    ...
    getList() {
      ...
      listUserBalance(this.queryParams).then(response => {
        this.userBalanceList = response.rows;
    ...
    handleUpdate(row) {
      ...
      getUserBalance(id).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateUserBalance(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addUserBalance(this.form).then(response => {
    ...
    handleExport() {
      this.download(this.exportUrl, {
        ...this.queryParams
      }, `userBalance_${new Date().getTime()}.xlsx`)
    },
  }
};
</script>
