    ...
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="类型" prop="type">
        <el-select v-model="queryParams.type" placeholder="请选择类型" clearable>
          <el-option
            v-for="dict in dict.type.collection_multimedia_type"
            :key="dict.value"
            :label="dict.label"
            :value="dict.value"
          />
...
export default {
  ...
  data() {
    return {
      ...
      form: {},
      // 导出地址
      exportUrl: 'manage/collectionMultimedia/export',
      // 表单校验
      rules: {
        collectionId: [
  ...
  methods: {
    ...
    getList() {
      ...
      listCollectionMultimedia(this.queryParams).then(response => {
        this.collectionMultimediaList = response.rows;
    ...
    handleUpdate(row) {
      ...
      getCollectionMultimedia(id).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateCollectionMultimedia(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addCollectionMultimedia(this.form).then(response => {
    ...
    handleExport() {
      this.download(this.exportUrl, {
        ...this.queryParams
      }, `collectionMultimedia_${new Date().getTime()}.xlsx`)
    },
  }
};
</script>
